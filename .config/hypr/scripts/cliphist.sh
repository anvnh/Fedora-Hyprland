...
#     ;;
"History")
    # selected_item=$(cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"History...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${roconf}" -kb-custom-1 "Ctrl+Return")
    selected_item=$(cliphist list | rofi -dmenu -theme-str "entry { placeholder: \"History...\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${roconf}" -kb-custom-1 "F2")

    if [ -n "$selected_item" ]; then
        if [ "$rofi_exit_code" -eq 10 ]; then
            # Shift + Enter was pressed (custom keybinding 1)
            full_item=$(echo "$selected_item" | cliphist decode)
            ...
            # Enter was pressed (default behavior)
            echo "$selected_item" | cliphist decode | wl-copy
            notify-send "Copied to clipboard."
        fi
    fi
...
"Delete")
    ...
    if [ "$rofi_exit_code" -eq 1 ]; then
        exec "$0"  # Run the script again to show the main menu 
    fi
    if [ -n "$selected_item" ]; then
...
"View Favorites")
    if [ -f "$favoritesFile" ] && [ -s "$favoritesFile" ]; then
        ...
        # Show the favorites list in rofi and wait for selection
        selected_favorite=$(printf "%s\n" "${decoded_lines[@]}" | rofi -dmenu -theme-str "entry { placeholder: \"View Favorites\";}" -kb-custom-1 "F2" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${roconf}")
        rofi_exit_code=$?

        if [ "$rofi_exit_code" -eq 1 ]; then
            exec "$0"  # Run the script again to show the main menu
        elif [ "$rofi_exit_code" -eq 10 ]; then  # Custom key "Delete" pressed
            if [ -n "$selected_favorite" ]; then
                # Find the index of the selected favorite
                index=$(printf "%s\n" "${decoded_lines[@]}" | grep -nxF "$selected_favorite" | cut -d: -f1)
                if [ -n "$index" ]; then
                    # Remove the selected favorite from the list
                    unset 'favorites[$((index - 1))]'
                    # Save the updated list back to the file
                    printf "%s\n" "${favorites[@]}" > "$favoritesFile"
                    notify-send "Favorite deleted."
                else
                    notify-send "Error: Selected favorite not found."
                fi
            fi
        elif [ -n "$selected_favorite" ]; then
            # Find the index of the selected favorite
            index=$(printf "%s\n" "${decoded_lines[@]}" | grep -nxF "$selected_favorite" | cut -d: -f1)
            # Use the index to get the Base64 encoded favorite
            if [ -n "$index" ]; then
...
"Manage Favorites")
    ...
    manage_action=$(echo -e "Delete from Favorites\nClear All Favorites" | rofi -dmenu -theme-str "entry { placeholder: \"Manage Favorites\";}" -theme-str "${r_scale}" -theme-str "${r_override}" -config "${roconf}")
